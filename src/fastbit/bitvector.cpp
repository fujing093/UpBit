...
std::vector<uint32_t> ibis::bitvector::decode(std::vector<uint32_t> &append_rowids) {
        ...
        if (*it > HEADER0) { // a fill
            ...
            if (*it >= HEADER1) { // 1-fill
                for (uint32_t i = 0; i < cnt; ++i) {
                    if (jnd + i > n_rows && append_rowids.size() > 0) {
                        rids.push_back(append_rowids[jnd + i - n_rows]);
                    } else {
            ...
            for (int i = MAXBITS; i >= 0; --i) {
                if ((*it >> i) & 1U) {
                    if (jnd + MAXBITS - i > n_rows && append_rowids.size() > 0) {
                        rids.push_back(append_rowids[jnd + MAXBITS - i - n_rows]);
                    } else {
...
void ibis::bitvector::setBit(const ibis::bitvector::word_t ind, int val) {
    ...
    if (m_vec.size() * MAXBITS == nbits) { // uncompressed
        ...
        if (enable_fence_pointer && index.size() > 0) {
            int indPos = -1;
            int vecPos = ind / MAXBITS;
            for (int i = 0; i < index.size(); ++i) {
                indPos = i - 1;
