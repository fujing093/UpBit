...
public:
    ...
    std::vector<uint32_t> decode(std::vector<uint32_t> &append_rowids);
...
private:
    ...
    inline void copy_runs(run &it, word_t &nw, const std::vector<std::pair<int, int>> &index,
                          int &bi_idx_pos); // copy nw words
...
inline void ibis::bitvector::copy_runs(run &it, word_t &nw, const std::vector<std::pair<int, int>> &index,
                                       int &bi_idx_pos) {
    // deal with the first word -- attach it to the last word in m_vec
    if (it.isFill != 0) {
        if (it.nWords > 1) {
